...
///
/// Used by PieceTable for the line-length index.  Each element represents one
/// line's length (including its terminator).  PrefixSum gives character offsets;
/// FindByPrefixSum maps offsets back to line numbers.
/// </summary>
public sealed class LineIndexTree {
    ...
    /// <summary>Adds <paramref name="delta"/> to the element at index <paramref name="i"/>.</summary>
    ...
    /// <summary>Inserts <paramref name="value"/> at 0-based index <paramref name="i"/>.</summary>
    ...
    /// <summary>Inserts multiple values starting at 0-based index <paramref name="i"/>.</summary>
    ...
    /// <summary>Creates a new tree from the given values.  O(n).</summary>
    ...
    /// <summary>Recomputes Size and Sum from children.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Builds a balanced treap from values in O(n) using a stack-based
    ...
    /// <summary>
    /// Builds a balanced subtree from a span of values for InsertRange.
    /// Uses the same Cartesian tree algorithm as BulkBuild.
    /// </summary>
    private int BuildBalanced(ReadOnlySpan<int> values, int lo, int hi) {
        if (lo > hi) return Nil;
        // Reuse BulkBuild for simplicity — allocate nodes and build.
