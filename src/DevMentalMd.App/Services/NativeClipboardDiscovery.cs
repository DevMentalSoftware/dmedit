...
/// On Windows: loads from the optional DevMentalMd.Print.Windows.dll via reflection.
...
public static class NativeClipboardDiscovery {
    ...
    private static INativeClipboardService? DiscoverWindows() {
        try {
            ...
            var dllPath = Path.Combine(dir, "DevMentalMd.Print.Windows.dll");
            ...
            var type = asm.GetType("DevMentalMd.Print.Windows.NativeClipboardService");
            if (type == null) {
